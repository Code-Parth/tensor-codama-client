...
export const WNS_DEPOSIT_NFT_DISCRIMINATOR = new Uint8Array([
  245, 148, 241, 58, 13, 253, 40, 195,
]);

export function getWnsDepositNftDiscriminatorBytes() {
  return fixEncoderSize(getBytesEncoder(), 8).encode(
    WNS_DEPOSIT_NFT_DISCRIMINATOR,
  );
}
...
export function getWnsDepositNftInstructionDataEncoder(): Encoder<WnsDepositNftInstructionDataArgs> {
  return transformEncoder(
    getStructEncoder([
      ["discriminator", fixEncoderSize(getBytesEncoder(), 8)],
      ["config", getPoolConfigEncoder()],
    ]),
    (value) => ({ ...value, discriminator: WNS_DEPOSIT_NFT_DISCRIMINATOR }),
  );
}

export function getWnsDepositNftInstructionDataDecoder(): Decoder<WnsDepositNftInstructionData> {
...
> {
  ...
  const instruction = {
    ...
    programAddress,
    data: getWnsDepositNftInstructionDataEncoder().encode(
